...
const businessRuleSchema = Joi.object({
  ...
  DISPUTE_EVIDENCE_VIDEO_MAX_FILE_SIZE_BYTES: Joi.number().min(1),
  STORAGE_MAX_FILE_SIZE_BYTES: Joi.number().min(1),
  BULL_QUEUE_VIDEO_PROCESSING_ATTEMPTS: Joi.number().min(0),
  BULL_QUEUE_VIDEO_PROCESSING_BACKOFF_DELAY: Joi.number().min(0),
  NOTIFICATION_RETENTION_DAYS: Joi.number().min(1),
  DISPUTE_RETENTION_DAYS: Joi.number().min(1),
  QUARANTINED_FILE_RETENTION_DAYS: Joi.number().min(1),
